...
from utils.server_utils import expose_port_ngrok

app = config.app
external_url = f"https://3.126.63.200:{config.SERVER_PORT}"  # expose_port_ngrok(config.SERVER_PORT)
telegram_webhook_base = f'{external_url}/telegram' # f"https://3.126.63.200:{config.SERVER_PORT}"
...
def init():
    data = {JSONKeys.server_id.value: config.THIS_SERVER_ID,
            ...
            JSONKeys.server_url.value: external_url}
...
def fetch_results(table_name, user_id):
    if table_name is None:
        return Response("Table name parameter not found", status=400)
    with db.get_engine().connect() as con:
        output = convert_list_of_dict_to_json(get_all_objects(con, table_name, user_id), ["id", "created", "updated"])
...
def fetch_excel(table_name, user_id):
    if table_name is None:
        return Response("Table name parameter not found", status=400)
    with db.get_engine().connect() as con:
        output = convert_list_of_dict_to_excel_table(get_all_objects(con, table_name, user_id), ["id", "updated"])
...
def fetch_csv(table_name, user_id):
    if table_name is None:
        return Response("Table name parameter not found", status=400)
    with db.get_engine().connect() as con:
        output = convert_list_of_dict_to_csv(get_all_objects(con, table_name, user_id), ["id", "updated"])
    Quiz.set_last_download(db.session,
                           Quiz.get_quiz_with_user_id_and_table_name(db.session, user_id, table_name).id,
                           datetime.datetime.utcnow())
...
def delete(request_body, user_id):
    ...
    elif request_body.get(JSONKeys.client_data.value):
        mappings = BotQuizMapping.get_mappings_by_user_id(db.session, user_id)
        for mapping in mappings:
            if mapping.bot_id in active_quiz_bots:
                active_quiz_bots[mapping.bot_id].discharge_bot()
                del active_quiz_bots[mapping.bot_id]
            Quiz.delete_quiz_with_user_id_and_table_name(db.session, user_id, mapping.table_name)
            BotQuizMapping.delete_mappings_with_user_id_and_table_name(db.session, user_id, mapping.table_name)
        with db.get_engine().connect() as con:
            drop_schema(con, user_id)
        return Response("Success", status=200)
    else:
        return Response("Not a valid delete request", status=400)
...
def fetch(request_body, user_id):
    ...
    elif fetch_what == FetchKeywords.results.value:
        return fetch_results(table_name, user_id)
